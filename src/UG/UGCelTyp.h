...
protected:	
	...
	// Draw cell that has highlighted text (found text support).
	// rect is bounding rect when isMultilineCell = false, left, top is text position.
	// rect is text area when isMultilineCell = true, left, top not used.
	//void DrawHighlightedText(CDC *dc, RECT *rect, int left, int top, LPCTSTR string, int stringLen, bool isMultilineCellint, int nLengthOfFoundText, int nCharIndexOfFoundText);

public:

	//general purpose routines
	//virtual int  DrawBitmap(CDC *dc,CBitmap * bitmap,RECT *rect,COLORREF backcolor);
	virtual void DrawBorder(CXeD2DRenderContext* pRctx,RECT *rect,RECT *rectout,CUGCell * cell);
	virtual void DrawText(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int offset,int col,long row,CUGCell *cell, int selected,int current);
...
public:
	...
	//void SetGridCtrlPtr(CUGCtrl* pCtrl) { m_ctrl = pCtrl; }
	...
	virtual void OnDraw(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int col,long row,CUGCell *cell,int selected,int current);
	//virtual BOOL OnDrawFocusRect(ID2D1RenderTarget* pRT,RECT *rect,int col,int row);
