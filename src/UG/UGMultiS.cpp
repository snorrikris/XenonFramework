...
int CUGMultiSelect::StartBlock(int col,long row)
{
	...
	if(m_mode&UG_MULTISELECT_ROW)
	{
		...
		m_currentItem->hiCol	=	m_GI->GetNumberCols();
	}
...
{	
	...
	//	m_GI->m_drawHintGrid.AddHint(maxLoCol,
	...
	//	m_GI->m_drawHintGrid.AddHint(oldEndCol,
	//		maxLoRow,m_currentItem->endCol,maxHiRow);
	//}
	//else if(m_currentItem->endCol < oldEndCol){
	//	m_GI->m_drawHintGrid.AddHint(m_currentItem->endCol,
...
int CUGMultiSelect::EnumFirstSelected(int *col,long *row)
{
	...
	if(m_mode&UG_MULTISELECT_OFF)
	{
		*col = m_GI->GetCurrentCol();
		*row = m_GI->GetCurrentRow();
		return UG_SUCCESS;
	}
	...
	m_enumStartCol	= m_GI->GetCurrentCol();
	m_enumStartRow	= m_GI->GetCurrentRow();
	m_enumEndCol	= m_GI->GetCurrentCol();
	m_enumEndRow	= m_GI->GetCurrentRow();
...
void CUGMultiSelect::OnLClick(int col,long row, UINT nFlags)
{
	...
	if(nFlags&MK_CONTROL){
		...
		//AddTotalRangeToDrawHints(&m_GI->m_drawHintGrid);
		ClearAll();
		StartBlock(col,row);
	}
}
