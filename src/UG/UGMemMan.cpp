...
int CUGMem::InsertCol(int col)
{
	...
	int numCols = m_GI->GetNumberCols();
	if(numCols > 0)
	{
		for(int loop = 0 ; loop < numCols;loop++)
		{
			if(m_GI->m_colInfo[loop].colTranslation == col)
			{
				col = loop;
				break;
			}
	...
	if(numCols > 0)
	{
		...
		{
			if(m_GI->m_colInfo[loop].colTranslation >= col)
				m_GI->m_colInfo[loop].colTranslation++;
		}
	}
...
//	m_findCol = *col;
//
//	long lastRow = m_GI->GetNumberRows()-1;
//	int  lastCol = m_GI->GetNumberCols()-1;
...
int CUGMem::SortBy(long startRow, long endRow, int *cols,int numCols,int flags)
{
	...
	while(OldListRowInfo != NULL && (rowCount < endRow) ){
		...
		rt = m_GI->OnSortEvaluate(cell1,cell2,flags);
		if(rt <= 0){
