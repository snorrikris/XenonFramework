...
//  Updated:    22 July 2006  by Hans Dietrich for XSuperTooltip
//
...
//{
//	template <typename T> struct Nut { virtual void f() { new T[1]; } };
...
//#else
	//#include "StdString.h"
#ifdef _UNICODE
#define CPPString	std::wstring	//non-MFC program UNICODE
#else
...
export class TT_Render
{
public:
	ID2D1RenderTarget* m_pRT = nullptr;
	IDWriteTextFormat* m_font = nullptr;
	ID2D1SolidColorBrush* m_text_brush = nullptr;
	ID2D1SolidColorBrush* m_bg_brush = nullptr;
	ID2D1SolidColorBrush* m_border_brush = nullptr;
};

/////////////////////////////////////////////////////////////////////////////
// CPPHtmlDrawer window

export class CPPHtmlDrawer
{
public:
	CPPHtmlDrawer(CXeUIcolorsIF* pUIcolors) : m_xeUI(pUIcolors)
	{
...
public:
	//Drawing methods
	void  PrepareOutput(LPCTSTR lpszHtml, LPSIZE lpSize) //Prepares to draw the HTML string
	{
	...
	//						  drawing area.
	////////////////////////////////////////////////////////////////////
	void  DrawPreparedOutput(TT_Render& rndr, LPCRECT lpRect)
	{
		//ASSERT(FALSE); // FIXME if needed
		...
		else
		{
			...
			while (nIndex < str.size())
			{
				...
				if (GetIndexNextAlphaNum(str, nIndex))
				{
					...
					{
						strName = str.substr(nBegin, nIndex - nBegin);
						if (!strName.empty())
						{
							if (chSymbol != _T(' '))
								nIndex--;
							...
							if (0 != chSymbol)
							{
								...
								if ((nIndex > nBegin) && (0 != chSymbol))
								{
									strProperty = str.substr(nBegin, nIndex - nBegin);
									SetTextStyle(strName.c_str(), strProperty.c_str());
								} //if
							} //if
						} //if
					} //if
	...
	void OnLButtonDown(LPPOINT lpClient, int nIDTool)
	{
		CPPTOOLTIP_TRACE("CPPHtmlDrawer::OnLButtonDown()\n");
	} //End OnLButtonDown

	BOOL OnSetCursor(LPPOINT lpClient)
	{
		CPPTOOLTIP_TRACE("in CPPHtmlDrawer::OnSetCursor\n");
		return FALSE;
	} //End OnSetCursor
	...
	static LPCTSTR GetVersionC() { return (LPCTSTR)L"1.3 beta"; }
...
protected:
	...
	DWORD GetTagFromList(CPPString sTagName, CPPString& strFullName, BOOL& bCloseTag) //Get tag from the list
	{
		...
		bCloseTag = (sTagName[0] == _T('/')) ? TRUE : FALSE;
		if (bCloseTag)
	...
	void ReplaceSpecChars()
	{
		...
		for (iter_mapStyles iter = m_mapSpecChars.begin(); iter != m_mapSpecChars.end(); ++iter)
		{
			sAlias = iter->first;
			sValue = iter->second;
			boost::replace_all(m_csHtmlText, sAlias, sValue);
		} //for

		boost::erase_all(m_csHtmlText, L"\r");
		if (!m_bEnableEscapeSequences)
		{
			//ENG: Remove escape sequences
			//RUS: Удаляем специальные символы
			boost::erase_all(m_csHtmlText, L"\n");
			boost::erase_all(m_csHtmlText, L"\t");
		}
		else
		{
			...
			boost::replace_all(m_csHtmlText, _T("\n"), _T("<br>"));
			boost::replace_all(m_csHtmlText, _T("\t"), _T("<t>"));
		} //if
	} //End of ReplaceSpecChars

	//The drawing methods
	void DrawHtml(TT_Render& rndr, LPSIZE lpSize, LPCRECT lpRect) //Draws the HTML text on device context or gets the size of the output area.
	{
	...
	//	CPPTOOLTIP_TRACE("m_defStyle.sFaceFont=%s =========\n"), m_defStyle.sFaceFont);
	//
	//	m_xeUI->ApplyTooltipFontAndSize(m_lfDefault);
	//
	//	// +++hd
	...
	//	{
	//		m_hFont = ::CreateFontIndirect(&m_lfDefault);
	//		CPPTOOLTIP_TRACE("m_lfDefault.lfFaceName=%s =========\n"), m_lfDefault.lfFaceName);
	//	}
	//	// new jms code ends
		...
		while (nIndex < m_csHtmlText.size())
		{
			...
			strText = m_csHtmlText.substr(nBegin, nIndex - nBegin);
	...
	//			strText = _T("<body>") + strText + _T("</body>");
				...
				szArea = DrawHtmlString(rndr, strText, &rcArea);

				//ENG: Updates a output area size
				//RUS: Обновляем размер области вывода
				lpSize->cx = std::max(lpSize->cx, szArea.cx);
				lpSize->cy += szArea.cy;
			...
			if (bFoundTable)
			{
				...
				//ENG: Output a table
				//RUS: Вывод таблицы
				szArea = DrawHtmlTable(rndr, strText, &rcArea);
	...
	SIZE DrawHtmlTable(TT_Render& rndr, CPPString& sTable, LPCRECT lpRect) //Draws the HTML table on device context or gets the size of the output area.
	{
		...
		SelectNewHtmlStyle(m_defStyle.strTag.c_str(), m_defStyle);
	...
	//			rcTable.bottom += m_defStyle.nBorderWidth * 2;
		...
		if (MODE_DRAW == m_nNumPass)
		{
			...
			{
				XeASSERT(FALSE);	// Fill background
		...
		if (szTable.cx && szTable.cy)
		{
			...
			for (i = 0; i < szTable.cy; ++i)
			{
				...
				if (SearchTag(sTable, nNewRow, _T("tr")))
				{
					...
					sRow = sTable.substr(nNewRow, nEndRow - nNewRow);

					//ENG: Draw a row of the table
					//RUS: Выводим строку таблицы
					DrawHtmlTableRow(rndr, sRow, &rcRow, cur_table, i);
				...
				for (int y = 0; y < szTable.cy; y++)
				{
					...
					for (int x = 0; x < szTable.cx; x++)
					{
						...
						if (sc.nColSpan == i)
						{
							if (i == 1)
							{
								cur_table.width[x] = std::max(cur_table.width[x], (int)sc.szCell.cx);
								if (sc.bFixedWidth)
			...
			for (i = 1; i <= szTable.cy; i++)
			{
				for (int y = 0; y < szTable.cy; y++)
				{
					...
					for (int x = 0; x < szTable.cx; x++)
					{
						...
						{
							if (i == 1)
								cur_table.height[y] = std::max(cur_table.height[y], (int)sc.szCell.cy);
							else
							{
								int span_height = 0;
								for (int z = 0; z < i; z++)
	...
	///////////////////////////////////////////////////////////////////////////////
	void DrawHtmlTableRow(TT_Render& rndr, CPPString& sRow, LPCRECT lpRect, STRUCT_TABLE& st, int nRow)
	{
		...
		while (nIndex < sRow.size())
		{
			...
			if (nNewCell < nEndRow)
			{
				...
				CPPString sCell = sRow.substr(nNewCell, nIndex - nNewCell);
	...
	//				rcCell.bottom -= m_defStyle.nPadding;
				...
				DrawHtmlTableCell(rndr, sCell, &rcCell, sc);
	...
	///////////////////////////////////////////////////////////////////////////////
	void DrawHtmlTableCell(TT_Render& rndr, CPPString& sCell, LPCRECT lpRect, STRUCT_CELL& sc) //Draws the HTML cell of the table
	{
		if (MODE_DRAW != m_nNumPass)
		{
			sc.szText.cx = 0;
	...
	//		sc.bHeightPercent = m_defStyle.bCellHeightPercent;
		...
		while (nIndex < sCell.size())
		{
			...
			int nNearlyTag = std::min(nEndCell, nNewTable);
			SIZE szTemp = { 0, 0 };
			if (nNearlyTag > nIndex)
			{
				...
				CPPString sText = sCell.substr(nIndex, nNearlyTag - nIndex);
				szTemp = DrawHtmlString(rndr, sText, &rcText);
				nIndex = nNearlyTag;
			} //if
			else if (nNewTable < nEndCell)
			{
				...
				CPPString sTable = sCell.substr(nNewTable, nIndex - nNewTable);
				szTemp = DrawHtmlTable(rndr, sTable, &rcText);
			}
			else
			{
				//ENG: Alas, it is the end of the cell
				//RUS: Конец ячейки
				nIndex = (int)sCell.size();
			} //if
			...
			if (MODE_DRAW != m_nNumPass)
			{
				...
				sc.szText.cx = std::max(szTemp.cx, sc.szText.cx);
		...
		if (MODE_DRAW != m_nNumPass)
		{
			//ENG: On first and second passes we are calculate the dimensions of the cell
			//RUS: На первом и втором проходах вычисляем размеры ячейки.
			sc.szCell.cx = std::max((LONG)m_defStyle.nCellWidth, sc.szText.cx);
			sc.szCell.cy = std::max((LONG)m_defStyle.nCellHeight, sc.szText.cy);
	...
	SIZE DrawHtmlString(TT_Render& rndr, CPPString& sHtml, LPCRECT lpRect) //Draws the HTML string on device context or gets the size of the output area.
	{
		SIZE szTextArea = { 0, 0 };
		...
		while (i < sHtml.size())
		{
			...
			if (!sText.empty())
			{
				...
				case TEXT_TRANSFORM_UPPERCASE:
					...
					sText = xet::to_upper(sText);
					break;
				case TEXT_TRANSFORM_LOWERCASE:
					//ENG: All chars make lower
					//RUS: Все символы переводим в нижний регистр
					sText = xet::to_lower(sText);
					break;
				case TEXT_TRANSFORM_CAPITALIZE:
					//ENG: Each first char of a word to upper
					//RUS: Кадый первый символ слова в верхний регистр, остальные в нижний
					sText = xet::to_lower(sText);
					for (nIndex = 0; nIndex < sText.size(); nIndex++)
					{
						if ((sText[nIndex] >= _T('a')) && (sText[nIndex] <= _T('z')))
						{
							if ((0 == nIndex) || (_T(' ') == sText[nIndex - 1]))
								sText[nIndex] = (TCHAR)(sText[nIndex] - _T('a') + _T('A'));
						} //if
					} //if
					break;
				} //switch
				...
				while (!sText.empty())
				{
					...
					sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, sText.c_str());
					...
					if (MODE_DRAW == m_nNumPass)
					{
						if (sz.cx)
						{
							if ((0 == (nRealWidth - sz.cx)) && (_T(' ') == sTemp[sTemp.size() - 1]))
							{
								...
								CSize szTemp = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, sTemp.c_str());
							...
							//ENG: Gets a size a output text
							//RUS: Получаем размер выводимого текста
							sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, sTemp.c_str());
							...
							//if (!bDrawOk)
							//{
							//	::TextOut(m_hDC, ptOutput.x, y, sTemp, stringLen > 2500 ? 2500 : stringLen);
							//}
							nSpacesInLine -= GetCountOfChars(sTemp);
					...
					//ASSERT(FALSE);
					break;	// TODO fix the text drawing
				} //while
			} //if
				...
				if (TAG_NONE != dwTag)
				{
						...
						if (StoreRestoreStyle(bCloseTag))
						{
							...
							if (!bCloseTag)
							{
								...
								case TAG_FONT:
									...
									while (nIndex < sProperties.size())
									{
										...
										if (!sParameter.empty())
										{
											...
											{
												if (((sValue == _T("transparent")) && sValue.empty()) || !m_bIsEnable)
												{
								...
								case TAG_BOTTOM:
									...
									break;
								case TAG_NEWSTYLE:
									SelectNewHtmlStyle(sTag.c_str(), m_defStyle);
									break;
								case TAG_SPAN:
									while (nIndex < sProperties.size())
									{
										...
										{
											std::wstring s1 = _T(".") + GetStyleString(sValue, _T(""));
											SelectNewHtmlStyle(s1.c_str(), m_defStyle);
										}
									} //while
									break;
								case TAG_HYPERLINK:
					...
					else
					{
						...
						case TAG_HLINE:
							...
							if (MODE_FIRSTPASS == m_nNumPass)
							{
								m_hline.nHeightLine = std::max(m_hline.nHeightLine, csTemp.nBorderWidth + 8);
						...
						case TAG_NEWLINE:
							...
							{
								sProperties = sProperties.substr(1);
...
//							} //if
						...
						case TAG_IMAGELIST:
							...
							//									if (!si.cx && !si.cy)
							//										si.cx = si.cy = std::min(sz.cx, sz.cy);
							//									else if (!si.cx)
							//										si.cx = si.cy;
							//									else if (!si.cy)
							//										si.cy = si.cx;
		...
		for (i = nFirstLine; i < m_nCurLine; i++)
		{
			...
			if (0 != m_hline.nAddPercentWidth)
			{
				...
				szTextArea.cx = std::max(szTextArea.cx, (LONG)m_hline.nWidthLine);
			} //if
		} //for
	//
	//	if (NULL != lpSize)
	...
	CPPString SearchNextTag(CPPString& str, CPPString& strTag, int& nIndex) //Search next tag
	{
		...
		while (nIndex < str.size())
		{
			...
			nIndex = (int)str.find(_T("<"), nIndex);
			if (nIndex < 0)
				nIndex = (int)str.size(); //A tag wasn't found
			sText += str.substr(nBegin, nIndex - nBegin);
			if (nIndex < str.size())
			{
				//May be it is a begin of the tag?
				if ((nIndex < (str.size() - 1)) && (_T('<') != str[nIndex + 1]))
				{
					//Yes of cause!!!
	...
	{
		strTag = _T("<") + strTag;
		while (nIndex < (int)str.size())
		{
			nIndex = (int)str.find(strTag, nIndex);
			if (nIndex < 0)
				nIndex = (int)str.size();
			else
			{
	...
	CPPString GetTagBody(CPPString& str, int& nIndex) //Gets a name of tag and the parameters of tag
	{
		...
		int nEndOfTag = (int)str.find(_T('>'), nIndex);
		...
		if (nEndOfTag > nIndex)
		{
			...
			sTagName = str.substr(nIndex, nEndOfTag - nIndex);
	...
	CPPString SplitTag(CPPString& sTag) //Split a tag to the tag's name and parameters
	{
		...
		if (tch != _T('\0'))
		{
			...
			sParam = sTag.substr(nIndex);
			sTag = sTag.substr(0, nIndex);
			xet::ltrim(sParam);
		} //if
		return sParam;
	} //End of SplitTag
	...
	CPPString GetNextProperty(CPPString& str, int& nIndex, CPPString& sProp) //Gets next property
	{
		...
		if (GetIndexNextAlphaNum(str, nIndex))
		{
			...
			sProp = str.substr(nBegin, nIndex - nBegin);
			...
			if (_T('=') == chFound)
			{
				...
				sValue = str.substr(nBegin, nIndex - nBegin);
				nIndex++;
			} //if
		} //if
		return sValue;
	...
	///////////////////////////////////////////////////////////////////////
	SIZE AnalyseCellParam(CPPString& sProperties, _STRUCT_CHANGESTYLE& cs, BOOL bTable)
	{
		SIZE szSpan = { 1, 1 };
		if (sProperties.empty())
			return szSpan;
		...
		while (i < sProperties.size())
		{
			...
			else if (sParameter == _T("border"))
			{
				...
				if (!cs.nBorderWidth)
					cs.nBorderStyle = 0; // CPPDrawManager::PEN_NULL;
			...
			else if (sParameter == _T("bgcolor"))
			{
				if (m_bIsEnable)
				{
					...
					if (cs.nFillBkgnd < 0)
						cs.nFillBkgnd = 0; // CPPDrawManager::EFFECT_SOLID;
				} //if
			}
	...
	int  VerticalAlignText(int y, int nHeight)
	{
		...
		if (MODE_FIRSTPASS == m_nNumPass)
		{
			...
			m_hline.nDescentLine = std::max(m_hline.nDescentLine, (int)(nHeight - tm.m_ascent));
			m_hline.nHeightLine = std::max(m_hline.nHeightLine, (int)tm.m_ascent);
		}
		else if (MODE_DRAW == m_nNumPass)
		{
			switch (m_defStyle.nVertAlign)
			{
			case ALIGN_VCENTER:
				y += (m_hline.nHeightLine - tm.m_height) / 2;
				break;
			case ALIGN_BASELINE:
				y += m_hline.nHeightLine - m_hline.nDescentLine - tm.m_ascent;
				break;
			case ALIGN_BOTTOM:
				y += m_hline.nHeightLine - tm.m_ascent;
				break;
			} //switch
		} //if
		return y;
	...
	int  VerticalAlignImage(int y, int nHeight)
	{
		...
		{
			//If calculate then don't output text
			m_hline.nHeightLine = std::max(m_hline.nHeightLine, nHeight);
		}
		else if (MODE_DRAW == m_nNumPass)
		{
			switch (m_defStyle.nVertAlign)
	...
	void Tag_NewLine(LPPOINT lpPoint, int nNum, LPSIZE lpSize)
	{
		...
		if (MODE_FIRSTPASS == m_nNumPass)
		{
			...
			if (!m_hline.nHeightLine)
				m_hline.nHeightLine = tm.m_height;
			lpSize->cx = std::max(lpSize->cx, lpPoint->x - m_rcOutput.left);
	...
	void  SearchEndOfRow(CPPString& str, int& nIndex) //Searching end of the row
	{
		...
		} while ((nIndex < str.size()) && nRow); //while
		nIndex -= 5;
	} //End SearchEndOfRow
	...
	wchar_t GetIndexNextChars(CPPString& str, int& nIndex, CPPString strChars)
	{
		...
		for (; nIndex < str.size(); nIndex++)
		{
			for (i = 0; i < strChars.size(); i++)
			{
				if (str[nIndex] == strChars[i])
					return str[nIndex];
			} //for
		} //for
		return 0;
	...
	wchar_t GetIndexNextNoChars(CPPString& str, int& nIndex, CPPString strChars)
	{
		...
		{
			bFound = FALSE;
			for (i = 0; (i < strChars.size()) && !bFound; i++)
			{
				if (str[nIndex] == strChars[i])
					bFound = TRUE;
			} //for
		...
		if (GetBeginParameter(str, nIndex, chBeginParam))
		{
			...
			if (0 != ch)
			{
				...
				if (_T('"') == str[nIndex])
				{
					...
					{
						nIndex++;
						return str.substr(nBegin + 1, nIndex - nBegin - 2);
					} //if
				}
				else
				{
					GetIndexNextChars(str, nIndex, strSeparators);
					return str.substr(nBegin, nIndex - nBegin);
				} //if
			} //if
		} //if
		return _T("");
	...
	CPPString GetWordWrap(CPPString& str, int nMaxSize, int& nRealSize)
	{
		...
		while ((sz.cx <= nRealSize) && (0 != tch))
		{
			...
			sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, str.c_str(), (size_t)nCurIndex);
		...
		if (0 == nLastIndex)
		{
			if (nMaxSize == nRealSize)
			{
				...
				for (i = 1; i < str.size(); i++)
				{
					...
					if (sz.cx > nRealSize)
					{
						sResult = str.substr(0, i);
						str = str.substr(i);
						sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, sResult.c_str(), (size_t)i);
						//::GetTextExtentPoint32(m_hDC, sResult, i, &sz);
						nRealSize = sz.cx;
						return sResult;
					} //if
				} //for
				sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, str.c_str(), (size_t)i);
		...
		{
			sResult = str.substr(0, nLastIndex + 1);
			str = str.substr(nLastIndex + 1);
			xet::rtrim(sResult);
			sz = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, sResult.c_str());
	...
	LPCTSTR GetTextStyle(LPCTSTR lpszStyleName)
	{
		...
		if (iterMap != m_mapStyles.end())
			return (LPCTSTR)iterMap->second.c_str();
	...
	{
		//Gets a string
		strStyle = xet::to_lower(strStyle);
		if (strStyle.empty())
			return;
		...
		while (nIndex < strStyle.size())
		{
			...
			if (GetIndexNextAlphaNum(strStyle, nIndex))
			{
				...
				if (0 != chSymbol)
				{
					...
					strName = strStyle.substr(nBegin, nIndex - nBegin);
					...
					{
						if (((strParameter == _T("transparent")) && strParameter.empty()) || !m_bIsEnable)
						{
	...
	{
		m_defStyle.strTag = L"";		//The name of the last opened tag

		//Font
		m_defStyle.nSizeFont = 20;		//The height of the logic font
		...
		m_defStyle.sFaceFont = _T("Segoe UI");  //The face name of the logic font

		LOGFONT* plf = nullptr; // GetSystemToolTipFont();
		//ASSERT(plf);
		...
		//Border
		m_defStyle.nBorderStyle = 0; // CPPDrawManager::PEN_NULL;	//The border style
		...
		m_defStyle.sHyperlink = L""; //The additional parameter for the link
	} //SetDefaultStyles

	BOOL GetStyleFontStyle(CPPString& str, BOOL bDefault)
	{
		if ((str == _T("normal")) || str.empty())
		{
			bDefault = FALSE;
		}
		...
		if (!str.empty())
		{
			if (str[0] == _T('#'))
			{
				if (str.size() == 7)
				{
					...
					strHex += str.substr(5, 2);
					strHex += str.substr(3, 2);
					strHex += str.substr(1, 2);
					crDefault = (COLORREF)std::stoul(strHex, 0, 0);
				} //if
			}
			else if ((str[0] >= '0') && (str[0] <= '9'))
				crDefault = (COLORREF)std::stoul(str, 0, 0);
			else
				crDefault = GetColorByName(str.c_str(), crDefault);
		} //if 
	...
	int StyleBorderWidth(CPPString& str, int nDefault)
	{
	...
	int GetLengthUnit(CPPString& str, int nDefault, BOOL bFont = FALSE)
	{
		...
		{
			//Percent value
			int percent = xen::stoi(str.substr(0, str.size() - 1));
		...
		int nSign = 0;
		if (str[0] == _T('+')) nSign = 1;
		else if (str[0] == _T('-')) nSign = -1;

		if (0 != nSign) str = str.substr(1);

		//ENG: This code fragment fixed by Reinhard Steiner(2004/10/20).
		int nValue = xen::stoi(str);
		CPPString strUnit;
		if (str.size() >= 2)
			strUnit = str.substr(str.size() - 2, 2);
		...
		else if (strUnit == _T("ex"))
		{
			...
			CSize szText = m_xeUI->GetTextSizeW(EXE_FONT::eUI_Font, strText.c_str());;
			//::GetTextExtentPoint32(m_hDC, strText, strText.GetLength(), &szText);
			nDefault = nValue * szText.cy;
		}
		else if (strUnit == _T("em"))	nDefault = nValue * tm.m_height;
		else
		{
			//ASSERT(FALSE); // FIXME if needed
			//Gets pixel in inch
			...
			{
				nValue = xen::stoi(str); // _tcstoul(str, 0, 0);//_ttoi(str);
	...
	int GetTableWidth(CPPString& str, int nClientWidth, int nMinWidth, BOOL bSet = FALSE)
	{
		if (!str.empty())
		{
			...
			while (i < str.size())
			{
				...
				strProperty = xet::to_lower(strProperty);
