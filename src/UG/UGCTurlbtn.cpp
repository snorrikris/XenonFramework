/*************************************************************************
				Class Implementation : CUGUrlBtnType
**************************************************************************/
...
/********************************************
CUGUrlBtnType - Constructor
*********************************************/
CUGUrlBtnType::CUGUrlBtnType(){
...
Return:
	cell type name
****************************************************/
LPCTSTR CUGUrlBtnType::GetName()
{
	return _T("Url button");
}
...
OnLClicked - overloaded CUGCellType::OnLClicked
	Checks to see if the url button has
	...
	url button) then the url button is 
	redrawn as raised and the UGCT_URLBUTTONCLICK
	notification is fired.

    **See CUGCellType::OnLClicked for more details
	about this function
*********************************************/
BOOL CUGUrlBtnType::OnLClicked(int col,long row,int updn,RECT *rect,POINT *point){
	...
	if(updn){
		...
		OnCellTypeNotify(m_ID,col,row,UGCT_URLBUTTONCLICK,0);
		m_btnDown = FALSE;
		m_btnCol = -1;
		m_btnRow = -1;			
...
OnDClicked - overloaded CUGCellType::OnDClicked
    ...
    **See CUGCellType::OnDClicked for more details
	about this function
*********************************************/
BOOL CUGUrlBtnType::OnDClicked(int col,long row,RECT *rect,POINT *point){
...
OnMouseMove - overloaded CUGCellType::OnMouseMove
	If the url button is depressed and the
    ...
    **See CUGCellType::OnMouseMove for more details
	about this function
*********************************************/
BOOL CUGUrlBtnType::OnMouseMove(int col,long row,POINT *point,UINT flags){
...
GetEditArea - overloaded CUGCellType::GetEditArea
    ...
    **See CUGCellType::GetEditArea for more details
	about this function
*********************************************/
int CUGUrlBtnType::GetEditArea(RECT *rect)
{
...
OnDraw - overloaded CUGCellType::OnDraw
	...
	the url button portion of the cell.

    **See CUGCellType::OnDraw for more details
	about this function
*********************************************/
void CUGUrlBtnType::OnDraw(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int col,long row,
							 CUGCell *cell,int selected,int current){

	XeASSERT(FALSE);	// Not implemented
...
//	if( style & UGCT_URLHIDEBUTTON && !current)
//	{
...
//
//	//draw the three ...
//	CPen bluepen( PS_SOLID, 1, RGB(0,0,128) );
//	dc->SelectObject( &bluepen );
//	COLORREF rgb = RGB(0,0,192);
//	if( style & UGCT_URLISEMAIL )
//		rgb = RGB(0,192,0);
//	CBrush bluebrush( rgb );
//	CBrush *oldbrush = (CBrush *)dc->SelectObject( &bluebrush );
//	CRect bluerect = rect;
//	int nCircleRadius = (bluerect.Height() * 60) / 100;
//	bluerect.left = bluerect.left + ((bluerect.Width() - nCircleRadius) / 2);
//	bluerect.right = bluerect.left + nCircleRadius + 1;
//	bluerect.top = bluerect.top + ((bluerect.Height() - nCircleRadius) / 2);
//	bluerect.bottom = bluerect.top + nCircleRadius + 1;
//	dc->Ellipse( bluerect );
...
Return:
	<none>
*****************************************************/
void CUGUrlBtnType::GetBestSize(CSize *size,CUGCell *cell)
{
