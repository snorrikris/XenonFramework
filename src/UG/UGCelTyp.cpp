...
****************************************************/
int CUGCellType::OnCellTypeNotify(long ID,int col,long row,long msg,long long param){
	return m_GI->OnCellTypeNotify(ID,col,row,msg,param);
}
...
Return
	<none>
****************************************************/
void CUGCellType::OnDraw(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int col,long row,CUGCell *cell,
...
//#endif
	...
	//				m_GI->m_drawHintGrid.AddHint(col+1,row);
	//				return;
	//			}
	//		}
	//		else{
...
//		nHeadColCount = m_GI->m_numberSideHdgCols;
//
...
//	if (size.cx > 0)
//		size.cx += (int) (m_GI->m_margin * fScale);
//
...
****************************************************/
//BOOL CUGCellType::OnDrawFocusRect(ID2D1RenderTarget* pRT,RECT *rect,int col,int row)
//{
...
//void CUGCellType::DrawHighlightedText(CDC * dc, RECT * rect, int left, int top, LPCTSTR string, 
//	int stringLen, bool isMultilineCell, int nLengthOfFoundText, int nCharIndexOfFoundText)
//{
//	const wchar_t* foundTextBeg = string + nCharIndexOfFoundText;
//	const wchar_t* foundTextEnd = foundTextBeg + nLengthOfFoundText;
//	if (isMultilineCell)
//	{
//		left = rect->left;
//		top = rect->top;
...
//	//CLineSplitter lineSplitter(string, stringLen, isMultilineCell);
//	//CSize sizeTest = dc->GetTextExtent("Test", 4);
//	//int nLineLen, cyLine = sizeTest.cy;
//	//for (UINT uLine = 0; uLine < lineSplitter.m_uNumLines; uLine++)
//	//{
//	//	const char *line = lineSplitter.GetLine(uLine, nLineLen);
//	//	const char *lineEnd = line + nLineLen;
//	//	if (foundTextBeg >= line && foundTextBeg < lineEnd)
//	//	{
//	//		// Draw left part.
//	//		int nLeftLen = (int)(foundTextBeg - line);
//	//		dc->ExtTextOut(left, top, ETO_CLIPPED, rect, line, nLeftLen, NULL);
//	//		CSize sizeLeftPart = dc->GetTextExtent(line, nLeftLen);
//
//	//		// Draw middle part (high light).
//	//		int nMiddPartLeft = left + sizeLeftPart.cx;
//	//		CSize sizeCenterPart = dc->GetTextExtent(foundTextBeg, nLengthOfFoundText);
//	//		CRect rcHighlight(nMiddPartLeft + 1, top, nMiddPartLeft + sizeCenterPart.cx/* - 1*/, top + sizeCenterPart.cy);
//	//		dc->FillSolidRect(rcHighlight, m_GI->m_xeUI->GetColor(CID::GrdHiLtTxtBg));
//	//		COLORREF rgbCurTxtFg = dc->SetTextColor(m_GI->m_xeUI->GetColor(CID::GrdHiLtTxtFg));
//	//		dc->ExtTextOut(nMiddPartLeft, top, ETO_CLIPPED, rect, foundTextBeg, nLengthOfFoundText, NULL);
//	//		dc->SetTextColor(rgbCurTxtFg);
//
//	//		// Draw right part (remainder).
//	//		int nRightLen = (int)(lineEnd - foundTextEnd);
//	//		dc->ExtTextOut(left + sizeLeftPart.cx + sizeCenterPart.cx, top, ETO_CLIPPED, rect, foundTextEnd, nRightLen, NULL);
//	//	}
//	//	else
//	//	{
//	//		dc->ExtTextOut(left, top, ETO_CLIPPED, rect, line, nLineLen, NULL);
//	//	}
//	//	top += cyLine;
//	//}
//}
...
Return:
	<none>
****************************************************/
void CUGCellType::DrawText(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int offset,int col,long row,CUGCell *cell,int selected,int current)
{
	...
	//if(overLapCol != col){
	//	CUGCell olCell;
	//	m_GI->GetCellIndirect(overLapCol,row,&olCell);
	...
	DrawBackground(pRctx, rect, backcolor );
	...
	if (properties & UGCELL_PNGIMG_SET)
	{
		//CXeGdiPng *pPng = m_GI->m_xeUI->GetPngImage(cell->GetPngImg());
		//CXeGdiPng *pPng = theUIstuff.GetCWF_Png(cell->GetPngImg());
		//if(pPng)
		{
			// Create GDI+ object - needed to draw PNG images.
			...
			//	rc1.bottom -= 2;
			//	rc1.left += 2;
			//	CSize size = pPng->DrawPng(graphics, rc1);
			//	//x = DrawBitmap(dc, cell->GetBitmap(), rect, backcolor);
			//	rect->left -= offset;
			//	offset += size.cx;
			//}
			//else {
				...
				D2D1_RECT_F rcImg = pRctx->_DrawButtonImage(pRT, cell->GetPngImg(), RectFfromRect(rc1),
					true, true, false);

				//rect->left += size.cx + 3;
				rect->left += (int)WidthOf(rcImg) + 3;
	...
	//size = m_GI->m_xeUI->GetTextSizeW(eFont, string, stringLen);
	//GetTextExtentPoint(dc->m_hDC,string,stringLen,&size);

	// horizontal text alignment
	DWRITE_TEXT_ALIGNMENT h_align = DWRITE_TEXT_ALIGNMENT_LEADING;
	if(alignment&UG_ALIGNCENTER)
	{
		h_align = DWRITE_TEXT_ALIGNMENT_CENTER;
		//left = rect->left + (rect->right - rect->left - size.cx) /2;
	}
	else if(alignment&UG_ALIGNRIGHT)
	{
		h_align = DWRITE_TEXT_ALIGNMENT_TRAILING;
		//left = rect->right - size.cx - m_GI->m_margin;
	}
	//else
	//{
	//	left = rect->left + m_GI->m_margin + offset;
	//}	

	// vertical text alignment
	DWRITE_PARAGRAPH_ALIGNMENT v_align = DWRITE_PARAGRAPH_ALIGNMENT_CENTER;
	...
	{
		v_align = DWRITE_PARAGRAPH_ALIGNMENT_FAR;
		//top = rect->bottom - size.cy - 1;
	}
	else
	{
		v_align = DWRITE_PARAGRAPH_ALIGNMENT_NEAR;
		//top = rect->top + 1;
	}
	...
	//			}
	//			else if(alignment & UG_ALIGNRIGHT) {
	//				tempRect.right -= m_GI->m_margin + offset;
	//				format |= DT_RIGHT;
	//			}
	//			else if(alignment & UG_ALIGNLEFT) {
	//				format |= DT_LEFT;
	//				tempRect.left += m_GI->m_margin + offset;
	//			}
	//		}
	...
	//		}
	//		
	//		//int nLengthOfFoundText, nCharIndexOfFoundText = cell->GetFoundTextOffset(nLengthOfFoundText);
	//		//if (nCharIndexOfFoundText >= 0)
	//		//{
	//		//	DrawHighlightedText(dc, tempRect, 0, 0, string, stringLen, true, nLengthOfFoundText, nCharIndexOfFoundText);
	//		//}
	//		//else
	//		{
	...
	//else{
		...
		//if (properties & (UGCELL_TIME_GAP_FLAG | UGCELL_TIME_JUMP_FLAG))
		//{
		//	bool isShowTimeGapsInGrid = m_GI->m_xeUI->GetUIsettingsFlag(UIF::SHOW_TIME_GAPS_IN_GRID);
		//	bool isShowTimeJumpsInGrid = m_GI->m_xeUI->GetUIsettingsFlag(UIF::SHOW_TIME_JUMPS_IN_GRID);
		//	bool isCellTimeGap = properties & UGCELL_TIME_GAP_FLAG;
		//	bool isCellTimeJump = properties & UGCELL_TIME_JUMP_FLAG;
		//	if ((isCellTimeGap && isShowTimeGapsInGrid)
		//		|| (isCellTimeJump && isShowTimeJumpsInGrid))
		//	{
		//		CBrush brush(isCellTimeGap ? m_GI->m_xeUI->GetColor(CID::WarnBg) : m_GI->m_xeUI->GetColor(CID::ErrorBg));
		//		CRect rcTop(rect->left, rect->top, rect->right, rect->top + ((rect->bottom - rect->top) / 8));
		//		dc->FillRect(rcTop, &brush);
		//	}
		//}

		// Note - there is a problem with ExTextOut - it sometimes fails to draw with very long strings.
		...
		//if (nCharIndexOfFoundText >= 0)
		//{
		//	DrawHighlightedText(dc, rect, left, top, string, stringLen, false, nLengthOfFoundText, nCharIndexOfFoundText);
		//	//// Draw left part.
		//	//dc->ExtTextOut(left, top, ETO_CLIPPED, rect, string, nCharIndexOfFoundText, NULL);
		//	//CSize sizeLeftPart = dc->GetTextExtent(string, nCharIndexOfFoundText);

		//	//// Draw middle part (high light).
		//	//COLORREF rgbCurTxtFg = dc->GetTextColor();
		//	//COLORREF rgbCurTxtBg = dc->GetBkColor();
		//	//dc->SetTextColor(m_GI->m_xeUI->GetColor(CID::GrdCellDefBg));
		//	//dc->SetBkMode(OPAQUE);
		//	//dc->SetBkColor(m_GI->m_xeUI->GetColor(CID::GrdCellDefTxt));
		//	//LPCSTR foundStr = string + nCharIndexOfFoundText;
		//	//dc->ExtTextOut(left + sizeLeftPart.cx, top, ETO_CLIPPED, rect, foundStr, nLengthOfFoundText, NULL);
		//	//dc->SetTextColor(rgbCurTxtFg);
		//	//dc->SetBkMode(TRANSPARENT);
		//	//dc->SetBkColor(rgbCurTxtBg);
		//	//CSize sizeCenterPart = dc->GetTextExtent(foundStr, nLengthOfFoundText);

		//	//// Draw right part (remainder).
		//	//LPCSTR remStr = string + nCharIndexOfFoundText + nLengthOfFoundText;
		//	//UINT remLen = stringLen - (nCharIndexOfFoundText + nLengthOfFoundText);
		//	//dc->ExtTextOut(left + sizeLeftPart.cx + sizeCenterPart.cx, top, ETO_CLIPPED, rect, remStr, remLen, NULL);
		//}
		//else
		{
			//BOOL bDrawOk = dc->ExtTextOut(left, top, ETO_CLIPPED, rect, string,
			//	stringLen > 5000 ? 5000 : stringLen,			// Note - ExtTextOut has a documented limit of 8192 string length but the limit seems to be lower at least on some computers.
			//	NULL);
			//if (!bDrawOk)
			//{
			//	dc->ExtTextOut(left, top, ETO_CLIPPED, rect, string,
			//		stringLen > 2500 ? 2500 : stringLen,
			//		NULL);

			//}

			//IDWriteTextFormat* font = m_GI->m_xeUI->D2D_GetFont(eFont);
			//XeD2D1_COLOR_F clrD2;
			//clrD2.SetFromRGB(textcolor, false);
			//Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> colorBrush = pRctx->GetD2SolidBrush(pRT, clrD2);
			//font->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_LEADING);
			//font->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
			CRect rcText(rect->left, rect->top, rect->right, rect->bottom);
			//pRT->DrawText(string, (UINT32)stringLen, font, RectFfromRect(rcText), colorBrush.Get());
			pRctx->_CreateAndDrawTextLayout(pRT, string, rcText, textcolor, eFont, h_align, v_align);
		}
	//}
...
Return:
	<none>
****************************************************/
void CUGCellType::DrawBackground(CXeD2DRenderContext* pRctx,RECT *rect,COLORREF backcolor)
{
	...
	ID2D1RenderTarget* pRT = pRctx->m_pCurrentRT;
	XeD2D1_COLOR_F clrD2;
	clrD2.SetFromRGB((COLORREF)backcolor, false);
	Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> colorBrush = pRctx->GetD2SolidBrush(pRT, clrD2);
	D2D1_RECT_F rcColor = RectFfromRect(*rect);
...
//	else
//		height = m_GI->m_topHdgHeights[((row * -1) -1)];
//
...
//		m_dScaleFactor = (((double)(rect->bottom - rect->top))/m_GI->m_numberTopHdgRows)/((double)height);
//	}
//
...
Returns:
	<none>
****************************************************/
void CUGCellType::DrawBorder(CXeD2DRenderContext* pRctx,RECT *rect,RECT *rectout,CUGCell * cell)
{
	ID2D1RenderTarget* pRT = pRctx->m_pCurrentRT;
	...
	//	dc->SelectObject(pen);
	//else
	//	dc->SelectObject(m_GI->m_xeUI->GetPen(CID::GrdExcelBrd));
	...
	if(style&15 || excelBdr){ //thin lines
		...
		if(style & 4 || excelBdr){ //right
			//if((style&4) ==0)
			//	origPen = (CPen*)dc->SelectObject(m_GI->m_xeUI->GetPen(CID::GrdExcelBrd));
		...
		pRT->DrawRectangle(RectFfromRect(*rect), pRctx->GetBrush(CID::GrdExcelBrd));
	}
	...
	{ //3D raised
		...
		for(loop=0;loop<m_GI->m_threeDHeight;loop++)
		{
			...
			//dc->LineTo(rect->left+loop,rect->top+loop);
			//dc->LineTo(rect->right-loop-1,rect->top+loop);
			pRT->DrawLine({ (float)(rect->left + loop), (float)(rect->bottom - loop - 1) },
					{ (float)(rect->left + loop), (float)(rect->top + loop) }, pRctx->GetBrush(CID::GrdHdrBrd));
			pRT->DrawLine({ (float)(rect->left + loop), (float)(rect->top + loop) },
					{ (float)(rect->right - loop - 1), (float)(rect->top + loop) }, pRctx->GetBrush(CID::GrdHdrBrd));
		}
		//dark color
		//dc->SelectObject(PO.m_pThreeDDarkPen);
		for(loop=0;loop<m_GI->m_threeDHeight;loop++)
		{
			...
			//dc->LineTo(rect->right-loop-1,rect->bottom-loop-1);
			//dc->LineTo(rect->left+loop,rect->bottom-loop-1);
			pRT->DrawLine({ (float)(rect->right-loop-1), (float)(rect->top+loop) },
					{ (float)(rect->right-loop-1), (float)(rect->bottom-loop-1) }, pRctx->GetBrush(CID::GrdHdrBrd));
...
Return:
	<none>
*****************************************************/
void CUGCellType::GetBestSize(CSize *size,CUGCell *cell)
{
	//select the font
	...
	if (cell->IsPropertySet(UGCELL_FONT_SET))
		font = cell->GetFont();
	//	oldFont = (HFONT)::SelectObject(hDC, cell->GetFont());
	//else
	//	oldFont = (HFONT)::SelectObject(hDC, m_GI->m_xeUI->GetFontHandle(EXE_FONT::eUI_Font));
	...
	if(cell->IsPropertySet(UGCELL_TEXT_SET))
	{
		...
		//const wchar_t* pStr = cell->GetText();
...
//		prevCol = col;
//		if(m_GI->m_defDataSource->GetPrevNonBlankCol(&prevCol,row) == UG_SUCCESS){
//	
//			//check to see if the text in the cell that was found
//			//is wider than the column that it is in
//			m_GI->GetCellIndirect(prevCol,row,cell);
//			// call GetBestSize for cell type set to this cell
//			size.cx = m_GI->GetColWidth( prevCol );
//			size.cy = m_GI->GetRowHeight( row );
//			(m_GI->GetCellType( cell->GetCellType()))->GetBestSize( dc, &size, cell );
//
//			//if the prev cell is wider than its column width
//			//and the cell allows overlapping
//
//			if(size.cx > m_GI->GetColWidth(prevCol) &&
//				m_GI->GetCellType(cell->GetCellType())->
//				CanOverLap(cell) != FALSE){
//
//				//check to see how many columns wide it is
//				width = 0;
//				for(loop = prevCol;loop <col;loop++){
//					width += m_GI->GetColWidth(loop);
//					if(width > size.cx){
//						break;
//					}
//				}
