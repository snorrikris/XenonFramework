...
***************************************************/
CUGVScroll::CUGVScroll(CXeUIcolorsIF* pUIcolors) : CXeScrollBar(pUIcolors)
{
	m_lastMaxTopRow = -2;
	m_lastScrollMode = -2;
	m_lastNumLockRow = -2;
...
/***************************************************
	Message handlers
***************************************************/
//BEGIN_MESSAGE_MAP(CUGVScroll, CXeScrollBar)
...
void CUGVScroll::Moved()
{
		...
		m_lastNumLockRow != m_GI->m_numLockRows)
	{
		...
		//else
		//{
		//	m_multiRange = (double)1001 / (double)m_GI->m_numberRows;
		//	m_multiPos   = (double)m_GI->m_numberRows / (double)1001;
		//	if(UG_SCROLLJOYSTICK==m_GI->m_vScrollMode)
		//	{
...
void CUGVScroll::VScroll(UINT nSBCode, UINT nPos) 
{
	...
	{
	case SB_LINEDOWN:
		m_GI->MoveTopRow(UG_LINEDOWN);
		break;
	case SB_LINEUP:
		m_GI->MoveTopRow(UG_LINEUP);
		break;
	case SB_PAGEUP:
		m_GI->MoveTopRow(UG_PAGEUP);
		break;
	case SB_PAGEDOWN:
		m_GI->MoveTopRow(UG_PAGEDOWN);
		break;
	case SB_TOP:
		m_GI->MoveTopRow(UG_TOP);
		break;
	case SB_BOTTOM:
...
LRESULT CUGVScroll::_OnRightDown(UINT nFlags, CPoint point)
{
