...
BOOL CUGCheckBoxType::OnLClicked(int col,long row,int updn,RECT *rect,POINT *point)
{
	...
	m_GI->GetCellIndirect( col, row, &m_cell );
	...
	if( updn )
	{
		...
		m_GI->SetCell(col,row,&m_cell);
		// notify the CUGCtrl derived class that the check state was changed
		OnCellTypeNotify(m_ID,col,row,UGCT_CHECKBOXSET,(long long)m_cell.GetNumber());
...
BOOL CUGCheckBoxType::OnCharDown(int col,long row,UINT *vcKey)
{
	...
	if ( *vcKey == VK_SPACE && m_cell.GetReadOnly() == FALSE )
	{
		...
		OnCellTypeNotify(m_ID,col,row,UGCT_CHECKBOXSET,(long long)val);
...
Return
	<none>
****************************************************/
void CUGCheckBoxType::OnDraw(CXeD2DRenderContext* pRctx, EXE_FONT eFont, RECT *rect,int col,long row,CUGCell *cell,int selected,int current)
{
	...
	DWORD button_state = cell->GetNumber() > 1 ? BST_INDETERMINATE
			: cell->GetNumber() > 0 ? BST_CHECKED : BST_UNCHECKED;
	...
	if ( selected || ( current && m_GI->m_currentCellMode & 2 ))
		DrawBackground(pRctx, rect, cell->GetHBackColor());
	else
		DrawBackground(pRctx, rect, cell->GetBackColor());
	...
	if( checkSize >= ( UGCT_CHECKMARGIN * 2 ) )
	{
		...
		{	
			...
			//	oldpen = (CPen*)dc->SelectObject((CPen*)CPen::FromHandle((HPEN)GetStockObject(WHITE_PEN)));
			//else
			//	oldpen = (CPen*)dc->SelectObject(m_GI->m_xeUI->GetPen(CID::GrdCbPen));
			//	//oldpen = (CPen*)dc->SelectObject((CPen*)CPen::FromHandle((HPEN)GetStockObject(BLACK_PEN)));
			...
			pRctx->_DrawCheckBox(pRT, RectFfromRect(checkrect), button_state, CID::CtrlTxt, CID::CtrlTxtDis, false);
		...
		//	if( cell->GetNumber() > 1 )
		//		oldpen = (CPen*)dc->SelectObject((CPen*)&m_darkPen);
		//	else if( selected || ( current && m_GI->m_currentCellMode&2 ))
		//		oldpen = (CPen*)dc->SelectObject((CPen*)CPen::FromHandle((HPEN)GetStockObject(WHITE_PEN)));
		//	else
		//		oldpen = (CPen*)dc->SelectObject(m_GI->m_xeUI->GetPen(CID::GrdCbPen));
		//		//oldpen = (CPen*)dc->SelectObject((CPen*)CPen::FromHandle((HPEN)GetStockObject(BLACK_PEN)));
	...
	if (!( style & UGCT_CHECKBOXUSEALIGN ))
	{
		...
		CUGCellType::DrawText(pRctx, eFont, rect,0,col,row,cell,selected,current);
	}	
...
Return:
	<none>
*****************************************************/
void CUGCheckBoxType::GetBestSize(CSize *size,CUGCell *cell)
{
	//select the font
	//HFONT oldFont = NULL;
	//get the best size
	...
	if (!( cell->GetCellTypeEx()&UGCT_CHECKBOXUSEALIGN ))
	{
		...
		if( cell->IsPropertySet(UGCELL_TEXT_SET))
		{
			...
			tempSize = m_GI->m_xeUI->GetTextSizeW(font, cell->GetLabelText());
			//const wchar_t* pStr = cell->GetLabelText();
