...
CPPDrawManager::CPPDrawManager(CXeUIcolorsIF* pUIcolors) : m_xeUI(pUIcolors)
{
...
//		
//		lighten += 1.0;
//		color_r = (BYTE)std::min((DWORD)GetRValue(clrColor) * lighten, 255.0);
//		color_g = (BYTE)std::min((DWORD)GetGValue(clrColor) * lighten, 255.0);
//		color_b = (BYTE)std::min((DWORD)GetBValue(clrColor) * lighten, 255.0);
//		clrColor = RGB(color_r, color_g, color_b);
///*		
//		lighten *= 255
...
//
//	int grainx2 = RAND_MAX/(std::max(1,2*granularity));
...
//			xs=rand()/(RAND_MAX/std::min(sXSize,sYSize))/2; //stroke lenght
...
//		} //for
//		//set vertical bump
//		d=std::min((long)16,sXSize/6);	//max edge=16
//		a=sYSize*sYSize/4;
//		posDst =iDst;
//		for(i = 0; i < sYSize; i++) {
//			y=i-sYSize/2;
...
//	{
//		right = std::min (x + (int)dwSrcWidth, nDestRight);
//		// Vertical copying
//		for (int y = nDestY; y < nDestBottom; y+= dwSrcHeight)
//		{
//			bottom = std::min (y + (int)dwSrcHeight, nDestBottom);
